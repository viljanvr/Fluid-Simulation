...
void advect(int N, int b, float *d, float *d0, float *u, float *v, float dt, Object **obstacle_mask, const std::vector<Object *>& obstacle_list) {
    int i, j, i0, j0, i1, j1;
    ...
    for (const auto& obj : obstacle_list) {
        auto intersection = obj->get_line_intersection(Vec2f((i - 0.5f) / N, (j - 0.5f) / N), Vec2f((x - 0.5f) / N, (y - 0.5f) / N));
...
void dens_step(int N, float *x, float *x0, float *u, float *v, float diff, float dt, Object **obstacle_mask, const std::vector<Object *> &obstacle_list) {
